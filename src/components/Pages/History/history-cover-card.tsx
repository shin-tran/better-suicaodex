  ...
  return (
    <Card className="relative rounded-sm shadow-none transition-colors duration-200 w-full h-full border-none bg-background">
      ...
      <CardFooter className="py-2 px-0 w-full flex flex-col gap-1 items-start">
        <Link href={`/manga/${chapter.manga.id}`} className="font-bold line-clamp-2 break-all">{chapter.manga.title}</Link>
        <Link href={`/chapter/${chapter.id}`} className="items-center flex gap-1">
          {chapter.language === "vi" ? (
            <VN className="size-4 shrink-0" />
          ) : (
            <GB className="size-4 shrink-0" />
          )}
          <span className="font-bold text-sm line-clamp-1 break-all hover:underline">{title}</span>
        </Link>
      </CardFooter>
    </Card>
  );
